...
```{r}
src <- Source('http://34.229.144.199:5000', 'h5serv')
rd <- Dataset(File(src, 'filetwo.subdirb.data.hdfgroup.org'), '/group0/group1/group2/data4d')
...
endpoint <- 'http://34.229.144.199:5000'
domain <- 'filetwo.subdirb.data.hdfgroup.org'
...
```{r}
f <- File(src, 'fileone.subdira.data.hdfgroup.org')
d <- Dataset(f, '/grpB/grpC/dataC')
...
```{r}
d <- Dataset(File(src, 'filetwo.subdirb.data.hdfgroup.org'), '/group0/group1/dataR')
...
TODO: fetching a single data point does not work, e.g.`getData(d, list(1:1, 4:4, 2:3, 3:3))` is OK, but `getData(d, list(1:1, 4:4, 2:2, 3:3))` times out.
